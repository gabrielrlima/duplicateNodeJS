        ...
        <Box sx={{ gap: 1, display: 'flex', alignItems: 'flex-start' }}>
          ...
          <Stack spacing={0.5}>
            <Box sx={{ gap: 1, display: 'flex', alignItems: 'center' }}>
              <Typography variant="h4">{propertyTitle || 'Imóvel'}</Typography>
              <Label
                variant="soft"
                color={
                  (status === 'Em andamento' && 'info') ||
                  (status === 'Pendente' && 'warning') ||
                  (status === 'Vendido' && 'success') ||
                  (status === 'Alugado' && 'error') ||
                  'default'
                }
              >
                {status}
        ...
        >
          ...
          {propertyId && (
            <Button
              ...
              href={paths.dashboard.property.edit(propertyId)}
            >
              Editar
            </Button>
          )}
